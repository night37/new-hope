...
export function Autocomplete({ option, onChange, formData }: autocompleteProps) {
            ...
            return new Promise<OptionType[]>((resolve) => {
                ...
                setTimeout(async () => {
                    const response:
                        ...
                        | AutocompleteNotFoundResponse = await query(option, inputValue, departementCode, regionCode);
    ...
    return (
            <div className="autocomplete_container w-full">
                ...
                <AsyncSelect
                    key={selectKey}
                    value={currentValue ? { value: currentValue.value, label: currentValue.label } : null}
