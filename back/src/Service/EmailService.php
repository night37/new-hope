...
class EmailService
{
    ...
    public function resendVerificationEmail(Request $request, UserRepository $userRepository, StructureRepository $structureRepository)
    {
        ...
        $structure = $structureRepository->findOneBy(['email' => $email]);
        // dd($structure->isVerified());
        if (!$user && !$structure) {
            self::displayMessage('danger', 'L\'adresse email n\'est pas associée à un compte.');
            return new RedirectResponse($this->router->generate('app_login'));
        }

        if (($user && $user->isVerified()) || ($structure && $structure->isVerified())) {
        ...
        $email = (new TemplatedEmail())
            ...
            ->to($user ? $user->getEmail() : $structure->getEmail())
